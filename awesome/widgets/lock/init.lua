  ...
  awful.spawn.easy_async_with_shell(
    ...
    function(stdout)
      if stdout:match('off') then
        label.markup = "Just Type The Password :)"
      else
        label.markup = "Careful, Your Caps Lock Is ON"
      end
    end
  )
end
    ...
    {
      {
        {
          {
            font = beautiful.font_sans .. " Light 110",
            format = "%H:%M",
            halign = "center",
            valign = "center",
          ...
          {
            font = beautiful.font_sans .. " Light 28",
            format = "%a, %d %B",
            halign = "center",
            valign = "center",
            widget = wibox.widget.textclock
